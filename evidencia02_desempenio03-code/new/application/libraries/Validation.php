...
class CI_Validation {
	...
	function run()
	{
		...
		foreach($_POST as $key => $val) {
			...
			$this->CI->tpl->assign($key, ( ! isset($_POST[$key])) ? '' : $this->prep_for_form($_POST[$key]));
		...
		foreach ($this->_rules as $field => $rules)
		{
			...
			{			
				if (in_array('isset', $ex, TRUE) OR in_array('required', $ex))
				{
					...
					$mfield = ( ! isset($this->_fields[$field])) ? $this->CI->lang->line('fd_' . $field) : $this->_fields[$field];
			...
			foreach ($ex As $rule)
			{
				...
				if ($result === FALSE)
				{
					...
					// Build the error message
					$mfield = ( ! isset($this->_fields[$field])) ? $this->CI->lang->line('fd_' . $field) : $this->_fields[$field];
					$mparam = ( ! isset($this->_fields[$param])) ? $this->CI->lang->line('fd_' . $param) : $this->_fields[$param];
		...
		$this->CI->tpl->assign('error_string', $this->error_string);
		return FALSE;
	}
	...
	// --------------------------------------------------------------------
	/**
	 * Get Fields that have error message
	 *
	 * @access public
	 * @return array
	 */
	...
	{
		if (preg_match('/^[2-9]{1}[0-9]{2}-[0-9]{3}-[0-9]{4}$/', $str))
		    return true;
		else
		    return false;
	}
	...
	function valid_time ($time) {
		...
		if (preg_match('/[012][0-9]:[0-6][0-9]/', $time))
		    return true;
		else
		    return false;
