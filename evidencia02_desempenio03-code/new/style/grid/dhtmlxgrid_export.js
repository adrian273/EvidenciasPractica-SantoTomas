dhtmlXGridObject.prototype.toPDF=function(url,mode,header,footer,rows){
	...
	function xml_top(profile) {
	    ...
	    var xml = "<?xml version=\"1.0\"?><rows profile='"+profile+"'";
	       if (header)
	          xml+=" header='"+header+"'";
	       if (footer)
	          xml+=" footer='"+footer+"'";
	    ...
	    for (var i=2; i < grid.hdr.rows.length; i++) {
	    	...
	    	for (var j=0; j < grid._cCount; j++) {
	    		...
	    		xml+="<column "+rpans+">"+grid.getColumnLabel(j,i-1)+"</column>"
	    	};
	    	xml+="</columns>";
	    };
	    xml+="</head>\n";
	...
	function xml_row(ind){
		...
		for (var i=0; i < grid._cCount; i++) {
			if ((!this._srClmn)||(this._srClmn[i])){
				var cell = grid.cells(r[i], i);
				xml+="<cell><![CDATA["+(cell.getContent?cell.getContent():(cell.getTitle?cell.getTitle():cell.getValue()))+"]]></cell>";
			}
		};
		return xml+"</row>";
	}
	...
	win.document.getElementById("grid_xml").value = btoa(xml_top(mode).replace("\u2013", "-") + xml_body() + xml_end());
	...
	for (var i = 0; i < this.hdr.rows[0].cells.length; i++){
		...
		out+="<column "+rpans+" width='"+this.getColWidth(i)+"' align='"+this.cellAlign[i]+"' type='"+this.cellType[i]
			...
			+(this.columnIds[i]
				...
				: "")+">";
		if (this._asCDATA)
			out+="<![CDATA["+this.getHeaderColText(i)+"]]>";
		else
			out+=this.getHeaderColText(i);
