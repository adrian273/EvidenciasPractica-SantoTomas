...
function dtmlXMLLoaderObject(funcObject, dhtmlObject,async,rSeed){
	...
	this.xmlDoc="";if(arguments.length==2)this.async=true;else

 this.async=async;this.onloadAction=funcObject||null;this.mainObject=dhtmlObject||null;this.waitCall=null;this.rSeed=rSeed||false;return this;};
 ...
 dtmlXMLLoaderObject.prototype.getXMLTopNode=function(tagName){
 	...
 	if(jsdebug==true)  alert(this.xmlDoc.responseText);

 	

 	if (this.xmlDoc.responseXML){var temp=this.xmlDoc.responseXML.getElementsByTagName(tagName);var z=temp[0];}else

 	var z=this.xmlDoc.documentElement;if (z){this._retry=false;return z;};if ((_isIE)&&(!this._retry)){var xmlString=this.xmlDoc.responseText;this._retry=true;this.xmlDoc = new ActiveXObject("Microsoft.XMLDOM");this.xmlDoc.async=false;this.xmlDoc.loadXML(xmlString);return this.getXMLTopNode(tagName);};

 	dhtmlxError.throwError("LoadXML","Incorrect XML",[this.xmlDoc,this.mainObject]);return document.createElement("DIV");
 ...
 dtmlXMLLoaderObject.prototype.loadXML = function(filePath,postMode,postVars,rpc) {
 	...
 	if ((!_isIE) && (window.XMLHttpRequest))
 		...
 		if (document.implementation && document.implementation.createDocument) { 
 			...
 			this.xmlDoc.onload = new this.waitLoadFunction(this); this.xmlDoc.load(filePath); return;
 	...
 	if (rpc){this.xmlDoc.setRequestHeader("User-Agent", "dhtmlxRPC v0.1 (" + navigator.userAgent + ")");
