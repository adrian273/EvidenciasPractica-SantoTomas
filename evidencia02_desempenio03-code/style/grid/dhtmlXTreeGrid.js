...
dhtmlXGridObject.prototype.doExpand=function(obj){
 ...
 var disp = "";
 if(obj.src.indexOf("arrow_right")!=-1){
...
dhtmlXGridObject.prototype.collapseKids=function(curRow,kidsAr,start){
 ...
 treeCell.innerHTML = treeCell.innerHTML.replace(/\/arrow_down\.gif/,"/arrow_right.gif")
}
...
dhtmlXGridObject.prototype.expandKids=function(curRow){
 ...
 treeCell = curRow.childNodes[this.cellType._dhx_find("tree")];
 treeCell.innerHTML = treeCell.innerHTML.replace(/\/(arrow_right|arrow_blank)\.gif/,"/arrow_down.gif");
 ...
 this.loadXML(this.kidsXmlFile+"/"+curRow.idd);
}
}
...
dhtmlXGridObject.prototype.iconImagePath = "";
...
dhtmlXGridObject.prototype.has_kids_dec=function(pRow){
 ...
 treeCell.innerHTML = treeCell.innerHTML.replace(/\/arrow_right|arrow_down\.gif/,"/arrow_blank.gif")
}
...
dhtmlXGridObject.prototype.has_kids_inc=function(pRow,treeCell){
 ...
 pRow.has_kids=1;
 pRow.expand="no";
 pRow.childNodes[treeCell].innerHTML = pRow.childNodes[treeCell].innerHTML.replace(/\/arrow_blank\.gif/,"/arrow_right.gif")
}
 else{
 pRow.has_kids++;
}
...
function eXcell_tree(cell){
 ...
 ul=this.grid.iconImagePath+"/"+url;
 img.src=url;
 ...
 preStr+= "<span class='space'><img src='"+this.grid.iconImagePath+"/blanc.gif' height='1px' class='space'></span>"
 ...
 pRow.childNodes[this.cell._cellIndex].innerHTML = pRow.childNodes[this.cell._cellIndex].innerHTML.replace(/\/arrow_right\.gif/,"/arrow_down.gif")
 ...
 node+="<img src='"+this.grid.iconImagePath+"/arrow_right.gif";
 else
 node+="<img width='9px' src='"+this.grid.iconImagePath+"/arrow_blank.gif";
 ...
 node+="<img hspace='7' src='"+this.grid.iconImagePath+"/"+valAr[3]+"' align='absmiddle' "+(this.grid._img_height?(" height=\""+this.grid._img_height+"\""):"")+(this.grid._img_width?(" width=\""+this.grid._img_width+"\""):"")+" >";
...
dhtmlXGridObject.prototype._fixLevel=function(r2){
 ...
 if(pRow){
 pRow.childNodes[trcol].innerHTML = pRow.childNodes[trcol].innerHTML.replace(/\/arrow_right\.gif/,"/arrow_down.gif")
 pRow.expand = "";
}
...
dhtmlXGridObject.prototype.addRow=function(new_id,text,ind,parent_id,img,child){
 ...
 text[trcol]=d[i]+"^"+def+"^"+(child?1:0)+"^"+(img||"arrow_blank.gif");
 if(d[i]!=0)
 if((!ind)||(ind==0)){
