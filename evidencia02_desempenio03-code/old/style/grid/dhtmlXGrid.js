...
//		this.xmlLoader.loadXML(url + "" + s + "rowsLoaded=" + this.getRowsNum() + "&lastid=" + this.getRowId(this.getRowsNum() - 1) + "&sn=" + Date.parse(new Date()));
	...
	this.loadXMLPost = function (url, form, afterCall) {
		if (this._dload) {
			this._dload = url;
			this._askRealRows();
			return true;
...
//		this.xmlLoader.loadXML(url + "" + s + "rowsLoaded=" + this.getRowsNum() + "&lastid=" + this.getRowId(this.getRowsNum() - 1) + "&sn=" + Date.parse(new Date()));
		this.xmlLoader.loadXML(url, "POST", sendVar);
	};
	this.setSkin = function (name) {
		this.entBox.className = "gridbox gridbox_" + name;
		this.enableAlterCss("ev_"+name,"odd_"+name)
		switch (name) {
		  case "xp":
			this._srdh = 22;
			break;
	...
	this.doLoadDetails = function (obj) {
		...
		if (root.tagName!="DIV") {
		 	...
		 	} else if (!self.xmlLoader.xmlDoc.nodeName) {
	...
	this.init = function (fl) {
		...
		this.setColumnIds();
		this.enableRowsHover(true, 'rowover');
		if (this.multiLine == -1) {
			this.multiLine = true;
		}
	...
	this.setSizes = function (fl) {
		if ((!this.noHeader) && ((!this.hdr.rows[0]) || (!this.hdrBox.offsetWidth))) {
			return;
		}
	...
	this._doContClick = function (ev) {
		...
		if (ev.button == 2) {
			...
			if (this._ctmndx) {
				if ((this.onBCM) && (!this.onBCM(el.parentNode.idd, el._cellIndex, this))) {
					return true;
				}
		...
		try {
			...
			if (ev.keyCode == 9 && !ev.shiftKey) {
				...
				if (ev.keyCode == 9 && ev.shiftKey) {
					...
					var arow = this.row;
					if (arow._childIndexes) {
						while ((aind >= 0) && (arow._childIndexes[aind] == arow._childIndexes[aind + 1])) {
							aind--;
						}
					}
					if (aind < 0) {
						...
						if (arow._childIndexes) {
							while (((arow._childIndexes[aind] != "0") && (!arow._childIndexes[aind])) && (aind >= 0)) {
								aind--;
							}
						}
						arow = this.rowsCol[this.rowsCol._dhx_find(this.row) - 1];
	...
	this._onHeaderClick = function (e) {
		...
		if ((this.grid.onHeaderClick) && (!this.grid.onHeaderClick(el._cellIndexS))) {
			return false;
		}
	...
	this.getHeaderColText = function (cin) {
		var z = this.hdr.rows[1];
		return z.cells[z._childIndexes ? z._childIndexes[parseInt(cin)] : cin].textContent;
	};
	this.setRowTextBold = function (row_id) {
		this.getRowById(row_id).style.fontWeight = "bold";
	};
	this.setRowStyle = function (row_id, css1) {
		this.getRowById(row_id).className = this.getRowById(row_id).className + " " + css1;
	};
	...
	this._serialiseConfig = function () {
		...
		for (var i = 0; i < this.hdr.rows[0].cells.length; i++) {
			...
			out += this.hdr.rows[1].cells[i].childNodes[0].nodeValue;
...
//				alert(this.cellType[r.childNodes[i]._cellIndex]);
			}
...
dhtmlXGridObject.prototype.setNumberFormat = function (mask, cInd, p_sep, d_sep) {
	...
	if (cInd) {
		this._ivizcol = cInd.split(",");
	}
	
	for (var i = 0; i < this._ivizcol.length; i++) {
		this._maskArr[this._ivizcol[i]] = [pfix, dfix, pref, postf, p_sep, d_sep];
	}
...
dhtmlXGridObject.prototype._parseHead = function (hheadCol) {
	...
	if (headCol.length > 0) {
		...
		for (var i = 0; i < headCol.length; i++) {
			...
			b_list += (headCol[i].getAttribute("type")||"ro")+",";
			c_list += (headCol[i].getAttribute("align")||"left")+",";
			d_list +=(headCol[i].getAttribute("sort")||"str")+",";
			...
			f_arr[i] = headCol[i].getAttribute("format");
 			...
 			h_list+=(headCol[i].getAttribute("resize")||"true")+",";
		}
		// formats array for each column
		this._formats = f_arr;
		...
		this.setColumnColor(e_list.substr(0, e_list.length - 1));
 		...
 		this.enableResizing(h_list.substr(0,h_list.length-1));
		for (var i = 0; i < headCol.length; i++) {
...
dhtmlXGridObject.prototype._fillRowFromXML = function (r, xml, tree, pId) {
	...
	for (var j = 0; j < cellsCol.length; j++) {
		...
		if (j != tree) {
			...
			strAr[strAr.length] = pId + "^" + cellVal + "^" + ((xml.getAttribute("xmlkids") || r._xml) ? "1" : "0") + "^" + (cellsCol[j].getAttribute("image") || "blank.gif");
		}
...
dhtmlXGridObject.prototype._drawTooltip = function (e) {
	...
	if (ced) {
    if((e ? e.target : event.srcElement).title.length>0)
    return true;
...
dhtmlXGridObject.prototype._createHRow = function (data, parent) {
	if (!parent) {
		...
		z.className = this.entBox.className + " ftr";
	...
	for (var i = 0; i < data[0].length; i++) {
		...
		w.innerHTML = this._aplNF(data[0][i], i);
...
dhtmlXGridObject.prototype.updateFromXMl=function(url, form,insert_new,reserved,afterCall) {
...
dhtmlXGridObject.prototype._refreshFromXML=function(xml) {
	...
	var hheadCol = xml.doXPath("//rows/head");
	if (hheadCol.length) {
		this._parseHead(hheadCol);
	}
	var tree = this.cellType._dhx_find("tree");
	...
	for (var i=0;i<rows.length;i++) {
		var row=rows[i];var id=row.getAttribute("id");
		if (this.rowsAr[id]) { 
			this._fillRowFromXML(this.rowsAr[id],row,tree);
		} else {
			var r=this._prepareRow(id);
			r=this._fillRowFromXML(r,row,tree);r = this._insertRowAt(r);
		};
	};
};
